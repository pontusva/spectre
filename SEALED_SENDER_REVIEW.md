...
|----|-----|--------|---------|
| NEW-HIGH-1 | HIGH | **OPEN — important** | Out-of-band verification (the entire C1 defense) is **not enforced**: `isVerified` is a DB boolean keyed by the relay *handle*, never bound to identity-key bytes, and is never consulted on the decrypt/display path (`message_service.dart`/`session_manager.dart` ignore it; only UI reads it). `Conversation.recipientPublicKey` is stored as `''`. So a verified flag survives an identity-key change (reinstall/rotation/relay-swap). **Fix: pin the verified identity-key bytes and gate attribution/display on a match; re-mark unverified on key change.** This must land for C1's mitigation to be real. |
